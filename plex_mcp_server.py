...
def create_starlette_app(mcp_server: Server, debug: bool = False, transport: str = 'sse'):
    ...
    Supports the SSE transport (/sse + /messages/) and the streamable-http
    transport (/mcp). Both share the same OAuth middleware and discovery routes.
    """
    lifespan = None
    ...
    if transport == 'streamable-http':
        ...
        # The session manager needs its task group running for the app's lifetime.
        lifespan = lambda app: mcp.session_manager.run()  # noqa: E731
    else:
...
def main():
    ...
    parser.add_argument('--host', default=os.environ.get('MCP_HOST', '0.0.0.0'),
                        help='Host to bind to (for sse/streamable-http, default: MCP_HOST env var or 0.0.0.0)')
    ...
    if args.transport == 'stdio':
        ...
        starlette_app = create_starlette_app(mcp_server, debug=args.debug, transport=args.transport)
        endpoint = "/sse" if args.transport == 'sse' else mcp.settings.streamable_http_path
        print(f"Starting {args.transport} server on http://{args.host}:{args.port}")
        print(f"Access the {args.transport} endpoint at {endpoint}")
